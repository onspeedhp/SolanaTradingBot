...
const config = {
  basePath:
    'https://cold-newest-market.solana-mainnet.quiknode.pro/5593ed4ac52c474bc8d5f1164249ff731839538b',
};
...
export class JupiterService {
  ...
  async checkTradableOnJupiter(mint: string) {
    ...
    try {
      ...
      if (is_buy) {
        ...
        total_fee_in_token = Number(
          (Number(quote.outAmount) * total_fee_percent_in_token).toFixed(0)
        );
      } else {
        total_fee_in_token = Number((fee * 10 ** decimal).toFixed(0));
        total_fee_in_sol = Number(
          (Number(quote.outAmount) * total_fee_percent_in_sol).toFixed(0)
        );
      }
